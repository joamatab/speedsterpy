...
class GdsTable(dict):
    ...
    def parseData(self, yamlDict: dict) -> None:
        ...
        for key, value in yamlDict.items():
            ...
            if "description" not in value.keys():
                raise TypeError(
                    'The parsed yamlDict must contain the "description" key'
                )
            self.add(
                key[0], key[1], value["name"], value["purpose"], value["description"]
            )
    ...
    def getGdsLayerDatatypeFromLayerNamePurpose(
        self, layerName: str, purpose: GdsLayerPurpose
    ) -> list:
        """_summary_
    ...
    def getDrawingMetalLayersMap(self, maxMetalNum=15) -> dict:
        ...
        met = "met"
        layerMap = {
            "met1": self.getGdsLayerDatatypeFromLayerNamePurpose(
                "met1", GdsLayerPurpose.DRAWING
            )
        }

        layerMap["via"] = self.getGdsLayerDatatypeFromLayerNamePurpose(
            "via", GdsLayerPurpose.DRAWING
        )
        for i in range(2, maxMetalNum):  # maximum number of metal layers
            aux = self.getGdsLayerDatatypeFromLayerNamePurpose(
                f"{met}{str(i)}", GdsLayerPurpose.DRAWING
            )
            ...
            lastVia = i
            aux = self.getGdsLayerDatatypeFromLayerNamePurpose(
                f"{via}{str(i)}", GdsLayerPurpose.DRAWING
            )
    ...
    def addBackannotation(self):
        ...
        purposeTable = self.getGdsTableEntriesFromPurpose(
        ...
        forbiddenLayerDatatypes = defaultdict(
            list
        )  # list of forbidden (layer, datatype) tuples
        ...
        for name, key in possibleLayerDatatypes.items():
            self.add(
                ...
                key[1],
                name,
                [GdsLayerPurpose.BACKANNOTATION.name],
                f"{name} backannotation",
            )
    ...
    def addHighlight(self):
        ...
        """
        # check if the backannotation layer is already present
        highlightingTable = self.getGdsTableEntriesFromPurpose(
        ...
        for layer, datatype in self.keys():
            if (layer + 1, 0) not in self.keys():
                self.add(
                    ...
                    0,
                    "highlight",
                    [GdsLayerPurpose.HIGHLIGHTING.name],
                    "net highlighting",
                )
